...
async function seed() {
  ...
  if (pgDataSource && pgDataSource.isInitialized) {
    ...
    for (const placeData of campusPlaces) {
      ...
      if (!pgPlace) {
        // Création avec le repository create (retourne une seule entité)
      ...
      if (sqliteCategory && pgPlace) {
        let sqlitePlace = await sqlitePlaceRepo.findOne({ where: { code: placeData.code } });
        if (!sqlitePlace) {
    ...
    console.log(`📊 Created ${campusCategories.length} categories`);
    console.log(`🏛️  Created ${campusPlaces.length} places`);
